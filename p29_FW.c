...
int main() {
	...
	mqd_t mqd_p28top29, mqd_p27top29, mqd_p29top31;
	int mq_return = 0;
	...
	/*control part*/
	mqd_t mqd_ctrltop29, mqd_p29toctrl;
	char ctrltop29[] = "/ctrltop29";
	char p29toctrl[] = "/p29toctrl";
	mqd_ctrltop29 = mq_open(ctrltop29, flags_ctrl, PERM, &attr_ctrl);
	check_return(mqd_ctrltop29, ctrltop29, "mq_open");
	mqd_p29toctrl = mq_open(p29toctrl, flags_ctrl, PERM, &attr_ctrl);
	check_return(mqd_p29toctrl, p29toctrl, "mq_open");
	...
	noti_tran.mqd_ctop = mqd_ctrltop29;
	noti_tran.mqd_ptoc = mqd_p29toctrl;
	noti_tran.qds = 2;
	noti_tran.i[0] = &i;
	noti_tran.i[1] = &j;
...
/////////////////////////////////////////////////////////////////////
	...
	while(1) {
		//p28top29
		mq_return = mq_getattr(mqd_p28top29, &q_attr);
		if(mq_return == -1) {
			printf("something wrong happened in %s when mq_getattr p28top29. \n", proname);
		}
		p_count1 = q_attr.mq_curmsgs >= 50?50:q_attr.mq_curmsgs;
		for(k = 0;k < p_count1;k++) {
			mq_return = mq_receive(mqd_p28top29, buffer, 2048, 0);
			if(mq_return == -1) {
			...
			if(((i + j)%SHOW_FREQUENCY == 0) || ((i + j) < SHOW_THRESHOLD)) {
				printf("%s:%s i = %lld, packet length = %d, pid = %d , working on CPU %d \n", proname, p28top29, i, mq_return, getpid(), getcpu());
			}
			if(i%CHECKQUEUE_FREQUENCY == 0) {

				checkqueue(mqd_p28top29, p28top29, &noti_tran);//check if the queue is congested and process needs to be splited.
			}
...
/////////////////////////////////////////////////////////////////////////
			...
			if(flag != 0) {
				//printf("i = %lld, flag: %d \n", i, flag);
					...
					mq_return = mq_send(mqd_p29top31, (char *) iph, mq_return, 0);
					if(mq_return == -1) {
						printf("%s:send %lld times fails:%s, errno = %d \n", p29top31, i, strerror(errno), errno);
						return -1;
					}
		...
		//p27top29
		mq_return = mq_getattr(mqd_p27top29, &q_attr);
		if(mq_return == -1) {
			printf("something wrong happened in %s when mq_getattr p28top29. \n", proname);
			return -1;
		}
		...
		for(k = 0;k < p_count2;k++) {
			mq_return = mq_receive(mqd_p27top29, buffer, 2048, 0);
			if(mq_return == -1) {
				printf("%s:%s receive %lld times fails:%s, errno = %d \n", proname, p27top29, j, strerror(errno), errno);
				return -1;
			}
			
			iph = (struct ndpi_iphdr *) buffer;
			if(((i + j)%SHOW_FREQUENCY == 0) || ((i + j) < SHOW_THRESHOLD)) {
				printf("%s:%s j = %lld, packet length = %d, pid = %d , working on CPU %d \n", proname, p27top29, j, mq_return, getpid(), getcpu());
			}
			if(j%CHECKQUEUE_FREQUENCY == 0) {

				checkqueue(mqd_p27top29, p27top29, &noti_tran);//check if the queue is congested and process needs to be splited.
			}
...
/////////////////////////////////////////////////////////////////////////
			...
			if(flag != 0) {
				printf("j = %lld, flag: %d \n", j, flag);
					...
					mq_return = mq_send(mqd_p29top31, (char *) iph, mq_return, 0);
					if(mq_return == -1) {
						printf("%s:send %lld times fails:%s, errno = %d \n", p29top31, j, strerror(errno), errno);
						return -1;
					}
	...
	//p28top29
	mq_return = mq_close(mqd_p28top29);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_close");
	mq_return = mq_unlink(p28top29);//returns 0 on success, or -1 on error.
	...
	//ctrltop29
	mq_return = mq_close(mqd_ctrltop29);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_close");
	mq_return = mq_unlink(ctrltop29);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_unlink");
	//p29toctrl
	mq_return = mq_close(mqd_p29toctrl);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_close");
	mq_return = mq_unlink(p29toctrl);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_unlink");
