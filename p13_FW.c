...
int main() {
	...
	mqd_t mqd_p12top13, mqd_p11top13, mqd_p13top15;
	int mq_return = 0;
	...
	/*control part*/
	mqd_t mqd_ctrltop13, mqd_p13toctrl;
	char ctrltop13[] = "/ctrltop13";
	char p13toctrl[] = "/p13toctrl";
	mqd_ctrltop13 = mq_open(ctrltop13, flags_ctrl, PERM, &attr_ctrl);
	check_return(mqd_ctrltop13, ctrltop13, "mq_open");
	mqd_p13toctrl = mq_open(p13toctrl, flags_ctrl, PERM, &attr_ctrl);
	check_return(mqd_p13toctrl, p13toctrl, "mq_open");
	...
	noti_tran.mqd_ctop = mqd_ctrltop13;
	noti_tran.mqd_ptoc = mqd_p13toctrl;
	noti_tran.qds = 2;
	noti_tran.i[0] = &i;
	noti_tran.i[1] = &j;
...
/////////////////////////////////////////////////////////////////////
	...
	while(1) {
		//p12top13
		mq_return = mq_getattr(mqd_p12top13, &q_attr);
		if(mq_return == -1) {
			printf("something wrong happened in %s when mq_getattr p12top13. \n", proname);
		}
		p_count1 = q_attr.mq_curmsgs >= 50?50:q_attr.mq_curmsgs;
		for(k = 0;k < p_count1;k++) {
			mq_return = mq_receive(mqd_p12top13, buffer, 2048, 0);
			if(mq_return == -1) {
			...
			if(((i + j)%SHOW_FREQUENCY == 0) || ((i + j) < SHOW_THRESHOLD)) {
				printf("%s:%s i = %lld, packet length = %d, pid = %d , working on CPU %d \n", proname, p12top13, i, mq_return, getpid(), getcpu());
			}
			if(i%CHECKQUEUE_FREQUENCY == 0) {

				checkqueue(mqd_p12top13, p12top13, &noti_tran);//check if the queue is congested and process needs to be splited.
			}
...
/////////////////////////////////////////////////////////////////////////
		...
		//p11top13
		mq_return = mq_getattr(mqd_p11top13, &q_attr);
		if(mq_return == -1) {
			printf("something wrong happened in %s when mq_getattr p12top13. \n", proname);
			return -1;
		}
		...
		for(k = 0;k < p_count2;k++) {
			mq_return = mq_receive(mqd_p11top13, buffer, 2048, 0);
			if(mq_return == -1) {
				printf("%s:%s receive %lld times fails:%s, errno = %d \n", proname, p11top13, j, strerror(errno), errno);
				return -1;
			}
			
			iph = (struct ndpi_iphdr *) buffer;
			if(((i + j)%SHOW_FREQUENCY == 0) || ((i + j) < SHOW_THRESHOLD)) {
				printf("%s:%s j = %lld, packet length = %d, pid = %d , working on CPU %d \n", proname, p11top13, j, mq_return, getpid(), getcpu());
			}
			if(j%CHECKQUEUE_FREQUENCY == 0) {

				checkqueue(mqd_p11top13, p11top13, &noti_tran);//check if the queue is congested and process needs to be splited.
			}
...
/////////////////////////////////////////////////////////////////////////
			...
			if(flag != 0) {
				printf("j = %lld, flag: %d \n", j, flag);
					...
					mq_return = mq_send(mqd_p13top15, (char *) iph, mq_return, 0);
					if(mq_return == -1) {
						printf("%s:send %lld times fails:%s, errno = %d \n", p13top15, j, strerror(errno), errno);
						return -1;
					}
	...
	//p12top13
	mq_return = mq_close(mqd_p12top13);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_close");
	mq_return = mq_unlink(p12top13);//returns 0 on success, or -1 on error.
	...
	//ctrltop13
	mq_return = mq_close(mqd_ctrltop13);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_close");
	mq_return = mq_unlink(ctrltop13);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_unlink");
	//p13toctrl
	mq_return = mq_close(mqd_p13toctrl);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_close");
	mq_return = mq_unlink(p13toctrl);//returns 0 on success, or -1 on error.
	check_return(mq_return, proname, "mq_unlink");
