...
int main() {
	...
	mqd_t mqd_send4top32;
	int mq_return = 0;
	char send4top32[]="/send4top32";

	mqd_send4top32 = mq_open(send4top32, flags, PERM, &attr);
	check_return(mqd_send4top32, proname, "mq_open");



	system("cp ../posix/traffic_sample.pcap /tmp/traffic_sample4.pcap");   //转移到tmpfs
	char errbuf[100];  //error buf for pcapReader

	pcap_t *pfile = pcap_open_offline("/tmp/traffic_sample4.pcap", errbuf);  //head
	if (pfile == NULL) {
		printf("%s\n", errbuf);
		return -1;
	} 
	...
	for(i = 0; i < PACKETS; i++){//9715
		...
		mq_return = mq_send(mqd_send4top32, (char *) iph, /*pkthdr->caplen*/40, 0);
		if(mq_return == -1) {
