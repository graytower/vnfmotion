...
int main() {
	...
	check_return(mqd_send1top8, proname, "mq_open");



	system("cp ../posix/traffic_sample.pcap /tmp/traffic_sample1.pcap");   //转移到tmpfs
	char errbuf[100];  //error buf for pcapReader

	pcap_t *pfile = pcap_open_offline("/tmp/traffic_sample1.pcap", errbuf);  //head
	if (pfile == NULL) {
		printf("%s\n", errbuf);
		return -1;
	} 
	...
	for(i = 0; i < PACKETS; i++){//9715
		...
		mq_return = mq_send(mqd_send1top8, (char *) iph, /*pkthdr->caplen*/40, 0);
		if(mq_return == -1) {
	...
	printf("send1 has sent %lld packets and is closing.\n", i);
